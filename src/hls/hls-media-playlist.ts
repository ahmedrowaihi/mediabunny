 ...
 */
import { videoRangeFromTransferCharacteristics } from '../misc';

import { languageToShortestForm } from '../misc';
import { getSegmentName } from '../dash/dash-misc';
...
export class MediaPlaylist {
	...
	getVideoRange(): string {
		...
		// Dolby Vision profile 8.4 may report 14, with the actual value in the SEI's
		// preferred_transfer_characteristic. shaka uses the compatible brand `db4g` as a workaround.
		if (tc === 14 && this.supplementalCodec && this.compatibleBrand === 'db4g') {
			return 'HLG';
		}

		return videoRangeFromTransferCharacteristics(tc) ?? '';
	}
	...
	build(options: {
		...
		if (!this.targetDurationSet) {
			// RFC 8216 §4.3.3.1 bounds each segment rounded to the nearest integer; shaka takes the ceiling
			this.setTargetDuration(Math.round(this.longestSegmentDurationSeconds));
		}

		let playlistType = this.hlsParams.playlistType;
		if (options.eventToVodOnEnd && options.endStream && playlistType === 'event') {
			playlistType = 'vod';
		}
