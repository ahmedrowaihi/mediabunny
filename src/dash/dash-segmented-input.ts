...
export class DashSegmentedInput extends SegmentedInput {
	...
	override getPairedSegmentedInputs() {
		...
		const ownTracks = tracks.filter(x => x.segmentedInput === this);
		const pairingMask = ownTracks.reduce((mask, x) => mask | x.pairingMask, 0n);
		// Representations of a type this one already has are alternatives to it, not played alongside it
		const paired = tracks
			.filter(x => (x.pairingMask & pairingMask) !== 0n && !ownTracks.some(y => y.info.type === x.info.type))
			.sort((a, b) => Number(!!b.segmentedInput.firstSegment) - Number(!!a.segmentedInput.firstSegment));

		return paired
			.filter((x, i) => paired.findIndex(y => y.info.type === x.info.type) === i)
			.map(x => x.segmentedInput);
	}
...
/**
 * A live presentation only offers the segments still inside its time-shift buffer, so the window is
 * bounded by `MPD@timeShiftBufferDepth` rather than by everything published since
 * `availabilityStartTime` — a stream running for months has long since dropped the early ones.
 *
 * ISO/IEC 23009-1 leaves the depth infinite when the attribute is absent, which no reader can
 * materialise; those fall back to a bounded window so a long-running stream degrades to "the recent
 * past" instead of enumerating millions of segments.
 */
const LIVE_WINDOW_WITHOUT_BUFFER_DEPTH = 3600;
...
const liveSegmentCount = (
	...
	const windowSec = Math.min(
		elapsedSec,
		context.timeShiftBufferDepth ?? LIVE_WINDOW_WITHOUT_BUFFER_DEPTH,
	);
	return Math.max(0, Math.floor((windowSec * timescale) / segDuration));
};
