...
import { bytesToBase64, escapeXmlText } from '../misc';
...
/**
 * FairPlay Streaming DRM system ID.
 *
 * @group Encryption
 * @public
 */
export const FAIRPLAY_SYSTEM_ID = '94ce86fb-07ff-4f43-adb8-93d2fa968ca2';
/**
 * Marlin Adaptive Streaming DRM system ID.
 *
 * @group Encryption
 * @public
 */
export const MARLIN_SYSTEM_ID = '5e629af5-38da-4063-8977-97ffbd9902d4';
...
export const buildPlayReadyObject = (
	kid: Uint8Array,
	options: {
		/** Protection scheme deciding the header version. Defaults to `cbcs`. */
		scheme?: ProtectionScheme;
		/** The content key, required for `cenc`/`cens` because their header carries a checksum over it. */
		key?: Uint8Array;
		/** Licence acquisition URL, written as `<LA_URL>`. */
		laUrl?: string;
	} = {},
): Uint8Array => {
	const scheme = options.scheme ?? 'cbcs';
	const kidBase64 = bytesToBase64(playReadyGuid(kid));
	const extra = options.laUrl !== undefined ? `<LA_URL>${escapeXmlText(options.laUrl)}</LA_URL>` : '';
	...
	if (scheme === 'cenc' || scheme === 'cens') {
		...
		const checksum = bytesToBase64(aesEcb(options.key, playReadyGuid(kid)).subarray(0, 8));
...
export const buildPlayReadyPssh = (
	...
	psshBox(PLAYREADY_SYSTEM_ID, buildPlayReadyObject(kid, options), []);
