...
import { FAIRPLAY_SYSTEM_ID, MARLIN_SYSTEM_ID, PLAYREADY_SYSTEM_ID } from '../crypto/pssh';
...
export const getCodecs = (mediaInfo: MediaInfo): string => {
	checkTrackCombination(mediaInfo);

	if (mediaInfo.videoInfo) {
		let codec = mediaInfo.videoInfo.codec ?? '';
		...
		// A multiplexed Representation lists every codec it carries, video first.
		return mediaInfo.audioInfo ? `${codec},${mediaInfo.audioInfo.codec ?? ''}` : codec;
	}
...
export const getAdaptationSetKey = (mediaInfo: MediaInfo, ignoreCodec: boolean): string => {
	...
	if (isMultiplexed(mediaInfo)) {
		// A multiplexed Representation must not share an AdaptationSet with a video-only one: a
		// player switching between them inside the set would lose the audio track mid-stream.
		key += `video+audio:${getBaseCodec({ ...mediaInfo, videoInfo: undefined })}:`;
	} else if (mediaInfo.videoInfo) {
		key += 'video:';
	} else if (mediaInfo.audioInfo) {
		key += 'audio:';
	} else if (mediaInfo.textInfo) {
		key += `${textTypeName(mediaInfo.textInfo.type ?? 'unknown')}:`;
		// A forced narrative track carries a different role and must not share an AdaptationSet with an
		// ordinary subtitle of the same language. Appended only when set, so an ordinary track's key is
		// unchanged from shaka's.
		if (mediaInfo.textInfo.forced) {
			key += 'forced:';
		}
	} else {
		key += 'unknown:';
...
/**
 * Precondition on `GetCodecs`, `GetSupplementalCodecs` and `GetSupplementalProfiles`.
 *
 * Video with audio is allowed; see {@link isMultiplexed} for why this deviates from shaka. Text is
 * addressed as its own AdaptationSet and still has to stand alone.
 *
 * @internal
 */
const checkTrackCombination = ({ videoInfo, audioInfo, textInfo }: MediaInfo): void => {
	if (textInfo && (videoInfo || audioInfo)) {
		throw new Error('MediaInfo must not carry textInfo alongside videoInfo or audioInfo');
	}
	if (!videoInfo && !audioInfo && !textInfo) {
		throw new Error('MediaInfo must carry at least one of videoInfo / audioInfo / textInfo');
	}
};
...
export const DRM_UUIDS = {
	MARLIN: MARLIN_SYSTEM_ID,
	FAIRPLAY: FAIRPLAY_SYSTEM_ID,
	PLAYREADY: PLAYREADY_SYSTEM_ID,
...
export const addContentProtectionElements = (
	mediaInfo: MediaInfo,
	parent: {
		/** Called once per descriptor, in the order the systems are listed. */
