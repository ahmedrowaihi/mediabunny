...
import { getSegmentName } from './dash-misc';
import type {
	ContainerType,
	MediaInfo,
	VideoInfo,
...
export class Representation {
	...
	init(): boolean {
		...
		// Video with audio is a multiplexed Representation; see `isMultiplexed` for the deviation.
		if (this.mediaInfo.textInfo && (this.mediaInfo.videoInfo || this.mediaInfo.audioInfo)) {
			return false;
		}
	...
	/**
	 * Set `availabilityTimeOffset` for LL-DASH: `target_segment_duration` minus the longest chunk a segment is
	 * published in, in seconds. Without `largestChunkDuration` that chunk is one frame, as in shaka's
	 * `SetAvailabilityTimeOffset`. No-op when the result is ≤ 0.
	 */
	setAvailabilityTimeOffset(largestChunkDuration?: number): void {
		const refTimeScale = this.mediaInfo.referenceTimeScale ?? 1;
		const chunkDurationSec = largestChunkDuration ?? this.frameDuration / refTimeScale;
		const ato = this.mpdOptions.mpdParams.targetSegmentDuration - chunkDurationSec;
		if (ato <= 0) {
			return;
		}
	...
	getXml(): RepresentationXmlNode | null {
		...
		if (!(this.codecs.length === 0)) {
			node.setStringAttribute('codecs', this.codecs);
		}
		...
		if (hasAudioInfo) {
			node.addAudioInfo(this.mediaInfo.audioInfo!);
		}

		node.addContentProtectionElements(this.contentProtectionElements);

		if (hasVodOnlyFields(this.mediaInfo)) {
			node.addVODOnlyInfo(
				this.mediaInfo,
		...
		if (hasLiveOnlyFields(this.mediaInfo)) {
			node.addLiveOnlyInfo(
				this.mediaInfo,
				this.segmentInfos,
	...
	private removeOldSegment(segmentInfo: SegmentInfo): void {
		...
		this.segmentsToBeRemoved.push(getSegmentName(
			...
			segmentStartTime,
			startNumber,
			this.mediaInfo.bandwidth ?? this.bandwidthEstimator.max(),
		));
