...
export class MasterPlaylist {
	...
	build(request: {
		...
		if (this.options.generatorBanner) {
			lines.push(this.options.generatorBanner);
		}

		if (this.options.independentSegments) {
			lines.push('');
			lines.push('#EXT-X-INDEPENDENT-SEGMENTS');
		}

		if (this.options.createSessionKeys) {
		...
		for (const t of audioTags) {
			...
			if (!languages.has(language)) {
				t.isDefault = !!language && language === this.options.defaultAudioLanguage;
				t.isAutoselect = true;
		...
		for (const t of subtitleTags) {
			...
			if (!languages.has(language)) {
				t.isDefault = !!language && language === this.options.defaultSubtitleLanguage;
				t.isAutoselect = true;
				languages.add(language);
			}
		...
		const closedCaptions = this.options.closedCaptions ?? [];
