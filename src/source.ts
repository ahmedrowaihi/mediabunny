...
export class CustomSource extends Source {
	...
	_read(
		...
		const readWithSize = (size: number) => {
			...
			// Callers check the range against the file size, but it was still unknown when they did
			if (end > size) {
				return null;
			}
		...
		return isThenable(result) ? result.then(readWithSize) : readWithSize(result);
	}
