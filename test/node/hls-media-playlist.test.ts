...
describe('MediaPlaylist — automatic target duration', () => {
	const targetDurationFor = (longestSeconds: number) => {
		const p = new MediaPlaylist(vodParams(), 'media.m3u8', 'video', 'group');
		p.setMediaInfo({ ...videoMediaInfo(), segmentTemplateUrl: 'segment-$Number$.m4s' });
		p.addSegment('segment-1.m4s', 0, 2 * TIME_SCALE, 0, 0);
		p.addSegment('segment-2.m4s', 2 * TIME_SCALE, Math.round(longestSeconds * TIME_SCALE), 0, 0);
		return /#EXT-X-TARGETDURATION:(\d+)/.exec(p.build({ endStream: true }))?.[1];
	};

	test('Is the longest segment rounded to the nearest integer', () => {
		expect(targetDurationFor(4.004)).toBe('4');
		expect(targetDurationFor(4.6)).toBe('5');
	});
});
