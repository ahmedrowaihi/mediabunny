...
`package.json` carries the upstream base version (currently `1.49.0`). The publish workflow strips any trailing `-beta.N`, then appends `-beta.<github.run_number>` to produce a monotonically increasing beta tag. Consumers should pin exact versions (`"@ahmedrowaihi/mediabunny": "1.49.0-beta.42"`) rather than tracking the floating `beta` dist-tag.
