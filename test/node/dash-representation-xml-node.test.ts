...
describe('RepresentationBaseXmlNode — content protection', () => {
	...
	test('AddContentProtectionElements emits one <ContentProtection> per descriptor with subelements', () => {
		...
		repr.addContentProtectionElements([widevine, clearkey]);
	...
	test('additionalAttributes overwrite top-level value/schemeIdUri (later-set-wins, no inline dedup)', () => {
		...
		const repr = new RepresentationXmlNode();
		repr.addContentProtectionElement(cp);
		expectXmlEqual(
			repr.toString(),
			'<Representation>'
	...
	test('addSupplementalProperty / addEssentialProperty render the right tag names', () => {
		...
		node.addSupplementalProperty('urn:scheme:supp', 'val1');
		node.addEssentialProperty('urn:scheme:ess', 'val2');
		expectXmlEqual(
			node.toString(),
			'<Test>'
	...
	test('descriptor without value omits value attribute', () => {
		...
		const node = new TestNode();
		node.addSupplementalProperty('urn:scheme:no-value', '');
		expectXmlEqual(
			node.toString(),
