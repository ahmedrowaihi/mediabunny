...
import { assert, MaybePromise, TransformationMatrix, UNDETERMINED_LANGUAGE } from '../misc';
...
export class DashDemuxer extends Demuxer {
	...
	readMetadata(): Promise<void> {
		return this.metadataPromise ??= (async () => {
			...
			const mpd = parseMpd(xml, { domParser: this.input._formatOptions.dash?.domParser });
...
abstract class DashInputTrackBacking extends SegmentedTrackBacking<InternalTrack> {
	...
	protected isPrimary(): boolean {
		return this.internalTrack.primary;
	}
}
