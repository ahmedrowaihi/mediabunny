...
import { FAIRPLAY_SYSTEM_ID, PLAYREADY_SYSTEM_ID, WIDEVINE_SYSTEM_ID } from '../../src/crypto/pssh.js';
import {
	capResolution,
	drm,
	dropByColorRange,
...
const dashManifest = (sets: MpdAdaptationSet[], mpdBaseURLs: string[] = []): Manifest => {
	const mpd: Mpd = {
		...
		serviceDescriptions: [],
		utcTiming: [],
		periods: [{ id: 'p0', start: null, duration: null, baseURLs: [], serviceDescriptions: [], adaptationSets: sets }],
	};
...
describe('manifest transforms', () => {
	...
	describe('quality atoms', () => {
		...
		test('capResolution sees dimensions declared on the AdaptationSet', () => {
			const manifest = dashManifest([
				set({ width: 3840, height: 2160, representations: [rep('uhd', 'hev1')] }),
				set({ width: 1280, height: 720, representations: [rep('sd', 'avc1')] }),
				set({ width: 3840, height: 2160, representations: [rep('override', 'avc1', { height: 720 })] }),
			]);
			const out = capResolution({ maxHeight: 1080 })(manifest);
			if (out.format !== 'dash') {
				throw new Error('expected dash');
			...
			const ids = out.mpd.periods[0]!.adaptationSets.flatMap(s => s.representations.map(r => r.id));
			expect(ids).toEqual(['sd', 'override']);
		});

		test('filterRenditions sees audioSamplingRate declared on the AdaptationSet', () => {
			const manifest = dashManifest([
				set({ contentType: 'audio', audioSamplingRate: 48_000, representations: [rep('a48', 'mp4a')] }),
				set({ contentType: 'audio', audioSamplingRate: 44_100, representations: [rep('a44', 'mp4a')] }),
			]);
			const out = filterRenditions(r => r.sampleRate === 48_000)(manifest);
			if (out.format !== 'dash') {
				throw new Error('expected dash');
			}
			const ids = out.mpd.periods[0]!.adaptationSets.flatMap(s => s.representations.map(r => r.id));
			expect(ids).toEqual(['a48']);
		});

		test('a kind-scoped bound leaves every other kind untouched', () => {
			const manifest = dashManifest([
				set({ representations: [
					rep('v-low', 'avc1', { bandwidth: 400_000 }),
					rep('v-high', 'avc1', { bandwidth: 3_000_000 }),
				] }),
				set({ contentType: 'audio', representations: [rep('a', 'mp4a', { bandwidth: 128_000 })] }),
			]);

			// A video floor must not drop audio that legitimately sits below it
			const out = filterBitrate({ min: 1_000_000, kind: 'video' })(manifest);
			if (out.format !== 'dash') {
				throw new Error('expected dash');
			}
			const ids = out.mpd.periods[0]!.adaptationSets.flatMap(s => s.representations.map(r => r.id));
			expect(ids).toEqual(['v-high', 'a']);

			// Unscoped, the same bound takes the audio with it
			const unscoped = filterBitrate({ min: 1_000_000 })(manifest);
			if (unscoped.format !== 'dash') {
				throw new Error('expected dash');
			}
			expect(unscoped.mpd.periods[0]!.adaptationSets.flatMap(s => s.representations.map(r => r.id)))
				.toEqual(['v-high']);
		});

		test('capResolution takes a floor as well as a ceiling', () => {
			const manifest = dashManifest([set({ representations: [
				rep('sd', 'avc1', { width: 640, height: 360 }),
				rep('hd', 'avc1', { width: 1280, height: 720 }),
				rep('uhd', 'avc1', { width: 3840, height: 2160 }),
			] })]);

			const out = capResolution({ minHeight: 480, maxHeight: 1080 })(manifest);
			if (out.format !== 'dash') {
				throw new Error('expected dash');
			}
			expect(out.mpd.periods[0]!.adaptationSets[0]!.representations.map(r => r.id)).toEqual(['hd']);
		});

		test('a codec filter can be scoped so it leaves other kinds alone', () => {
			const ladder = dashManifest([
				set({ representations: [rep('v-720', 'avc1.640028'), rep('v-1080', 'avc1.640032')] }),
				set({ contentType: 'audio', representations: [rep('a-lc', 'mp4a.40.2'), rep('a-he', 'mp4a.40.5')] }),
			]);
			const ids = (manifest: Manifest) => manifest.format === 'dash'
				? manifest.mpd.periods[0]!.adaptationSets.flatMap(s => s.representations.map(r => r.id))
				: [];

			// Scoped to audio, the video ladder is untouched
			expect(ids(keepCodecs(['mp4a.40.2'], 'audio')(ladder))).toEqual(['v-720', 'v-1080', 'a-lc']);

			// Unscoped, every video rendition fails the list and is dropped with it
			expect(ids(keepCodecs(['mp4a.40.2'])(ladder))).toEqual(['a-lc']);

			// dropCodecs scopes the same way
			expect(ids(dropCodecs(['avc1'], 'audio')(ladder))).toEqual(['v-720', 'v-1080', 'a-lc', 'a-he']);
		});

		test('a codec string with a profile separates AAC-LC from both HE-AAC profiles', () => {
			// mp4a.40.29 (HE-AAC v2) is the case a prefix match gets wrong: it starts with 'mp4a.40.2'
			const manifest = dashManifest([set({ contentType: 'audio', representations: [
				rep('aac-lc', 'mp4a.40.2'),
				rep('he-aac', 'mp4a.40.5'),
				rep('he-aac-v2', 'mp4a.40.29'),
			] })]);
			const ids = (m: Manifest) => m.format === 'dash'
				? m.mpd.periods[0]!.adaptationSets[0]!.representations.map(r => r.id)
				: [];

			expect(ids(keepCodecs(['mp4a.40.2'])(manifest))).toEqual(['aac-lc']);
			expect(ids(keepCodecs(['mp4a.40.29'])(manifest))).toEqual(['he-aac-v2']);
			expect(ids(keepCodecs(['mp4a.40.5', 'mp4a.40.29'])(manifest))).toEqual(['he-aac', 'he-aac-v2']);

			// dropCodecs shares the same matcher, so the collision would show up inverted there
			expect(ids(dropCodecs(['mp4a.40.2'])(manifest))).toEqual(['he-aac', 'he-aac-v2']);

			// A bare fourcc still matches every profile, as it always has
			expect(ids(keepCodecs(['mp4a'])(manifest))).toEqual(['aac-lc', 'he-aac', 'he-aac-v2']);
		});
...
`;
	...
	describe('drm', () => {
		...
		const options = {
			scheme: 'cbcs' as const,
			defaultKid: hexStringToBytes('1234567890abcdef1234567890abcdef'),
			systems: [
				{ uuid: WIDEVINE_SYSTEM_ID, pssh: psshBox(WIDEVINE_SYSTEM_ID, widevinePssh) },
		...
		test('DASH: prepends mp4protection + per-system ContentProtection, skips FairPlay', () => {
			...
			expect(text).toContain(`schemeIdUri="urn:uuid:${WIDEVINE_SYSTEM_ID}"`);
			expect(text).toContain(`schemeIdUri="urn:uuid:${PLAYREADY_SYSTEM_ID}"`);
			expect(text).not.toContain(FAIRPLAY_SYSTEM_ID); // DASH cannot signal FairPlay
		...
		test('HLS media: one #EXT-X-KEY per system (widevine data, playready PRO, fairplay skd)', () => {
			...
			expect(text).toContain(`KEYFORMAT="urn:uuid:${WIDEVINE_SYSTEM_ID}"`);
