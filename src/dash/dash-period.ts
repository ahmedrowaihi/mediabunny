...
export class Period {
	...
	getXml(outputPeriodDuration: boolean): XmlNode | null {
		...
		if (this.mpdOptions.mpdParams.lowLatencyDashMode) {
			...
			serviceDescription.setIntegerAttribute('id', this.idValue);
			const latency = new XmlNode('Latency');
			const targetLatencyMs = Math.floor(this.mpdOptions.mpdParams.targetLatencySeconds * 1000);
			latency.setIntegerAttribute('target', targetLatencyMs);
			serviceDescription.addChild(latency);
			period.addChild(serviceDescription);
		}
		...
		if (outputPeriodDuration) {
			period.setStringAttribute('duration', secondsToXmlDuration(this.durationSecondsValue));
		} else if (this.mpdOptions.mpdType === 'dynamic') {
			period.setStringAttribute('start', secondsToXmlDuration(this.startTimeInSeconds));
		}
		return period;
	}
