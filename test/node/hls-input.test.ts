...
test.concurrent('Live HLS', { timeout: 30_000 }, async () => {
	...
	const duration = await Promise.race([
		...
		// Measured at 1.1-1.6 s against the live stream; 2 s left too little headroom for a network hiccup.
		// The 2 s race above is what proves the non-skipping call hangs, so this one only needs to beat "forever".
		rejectAfter(5000, 'Baby you make me smile'),
	]);
...
test.concurrent('Missing media tag codec', { timeout: 15_000 }, async () => {
	using input = new Input({
...
test.concurrent('Widevine encryption (SAMPLE-AES-CTR) fails without keys', { timeout: 15_000 }, async () => {
	using input = new Input({
...
test.concurrent('Widevine encryption (SAMPLE-AES-CTR) succeeds with string keys', { timeout: 15_000 }, async () => {
...
test.concurrent('Widevine encryption (SAMPLE-AES-CTR) succeeds with buffer keys', { timeout: 15_000 }, async () => {
	const keyMap = new Map([
...
test.concurrent('Widevine HLS passes #EXT-X-KEY PSSH boxes to key resolver', { timeout: 15_000 }, async () => {
	const keyMap = new Map([
		['4d97930a3d7b55fa81d0028653f5e499', '429ec76475e7a952d224d8ef867f12b6'],
		['d21373c0b8ab5ba9954742bcdfb5f48b', '150a6c7d7dee6a91b74dccfce5b31928'],
		['6f1729072b4a5cd288c916e11846b89e', 'a84b4bd66901874556093454c075e2c6'],
...
test.concurrent('SourceRequest.isRoot', { timeout: 15_000 }, async () => {
	using input = new Input({
		source: new CustomPathedSource(
