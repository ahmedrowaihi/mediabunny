		...
		while (true) {
			...
			try {
				...
				if (partDuration === undefined) {
					...
					let nextPartBoundary = playlist.currentSegmentStartTimestamp + partDuration;
					let videoIndex = 0;
					let audioIndex = 0;
					...
					while (videoIndex < videoEndIndex || audioIndex < audioEndIndex) {
						...
						// Video decides the cuts when present, so a part can begin on one of its key frames
						const cutsPart = partStarts.length > 0
							&& (isVideo || !videoTrack)
							&& packet.timestamp >= nextPartBoundary;

						if (partStarts.length === 0 || cutsPart) {
							if (cutsPart) {
								partEnds.push(await muxer.forceFragmentFinalization());
							}
							while (nextPartBoundary <= packet.timestamp) {
								nextPartBoundary += partDuration;
							}
