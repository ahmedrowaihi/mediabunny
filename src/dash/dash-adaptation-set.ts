...
export class AdaptationSet {
	...
	getXml(): AdaptationSetXmlNode | null {
		...
		node.setStringAttribute('contentType', this.contentTypeValue);
		...
		if (widths.length === 1) {
			...
			node.setIntegerAttribute('width', widths[0]!);
		} else if (widths.length > 1) {
			node.setIntegerAttribute('maxWidth', widths[widths.length - 1]!);
		}

		if (heights.length === 1) {
			suppressRepresentationHeight = true;
			node.setIntegerAttribute('height', heights[0]!);
		} else if (heights.length > 1) {
			node.setIntegerAttribute('maxHeight', heights[heights.length - 1]!);
		}

		if (this.subsegmentStartWithSAP) {
			node.setIntegerAttribute('subsegmentStartsWithSAP', this.subsegmentStartWithSAP);
		} else if (this.startWithSAP) {
			node.setIntegerAttribute('startWithSAP', this.startWithSAP);
		}
		...
		if (frameRateKeys.length === 1) {
			...
			node.setStringAttribute('frameRate', this.videoFrameRates.get(frameRateKeys[0]!)!);
		} else if (frameRateKeys.length > 1) {
			node.setStringAttribute(
				'maxFrameRate',
		...
		if (this.segmentsAligned === 'true') {
			const attrName = this.mpdOptions.dashProfile === 'onDemand'
				? 'subsegmentAlignment'
				: 'segmentAlignment';
			node.setStringAttribute(attrName, 'true');
		}
		...
		for (const accessibility of this.accessibilities) {
			node.addAccessibilityElement(accessibility.scheme, accessibility.value);
		}
		...
		for (const role of orderedRoles) {
			if (this.rolesValue.has(role)) {
				node.addRoleElement('urn:mpeg:dash:role:2011', roleToText(role));
			}
		}

		if (this.label.length > 0) {
			node.addLabelElement(this.label);
		}
	...
	private updateFromMediaInfo(mediaInfo: MediaInfo): void {
		...
		if (mediaInfo.videoInfo) {
			...
			if (mediaInfo.textInfo.forced) {
				this.rolesValue.add('forcedSubtitle');
			}
		}
	}
