...
import { BandwidthEstimator } from '../../src/bandwidth-estimator.js';
...
import { getSegmentName, substituteTemplate } from '../../src/dash/dash-misc.js';
...
describe('a segment template names the same file on both sides', () => {
	// The writer builds the name, the demuxer resolves the URL. Two engines that disagree put a
	// literal `$Number%05u$` in a request for a file written as `00042`.
	const templates = [
		'seg-$Number$.m4s',
		'seg-$Number%05d$.m4s',
		'seg-$Number%05u$.m4s',
		'seg-$Number%d$.m4s',
		'seg-$Time%08d$.m4s',
		'$Bandwidth$/seg-$Number%03d$.m4s',
		'literal-$$-seg-$Number$.m4s',
	];

	test.each(templates)('%s', (template) => {
		const written = getSegmentName(template, 900, 42, 1000);
		const resolved = substituteTemplate(template, { number: 42, time: 900, bandwidth: 1000 });
	...
	test('an identifier the writer has no value for is refused, not invented', () => {
		expect(() => getSegmentName('$RepresentationID$-$Number$.m4s', 0, 1, 1000))
			.toThrow(/Unknown segment-template identifier/);

		expect(substituteTemplate('$RepresentationID$-$Number$.m4s', { number: 1, representationId: 'v0' }))
			.toBe('v0-1.m4s');
	});

	test('an unbalanced template is refused by both', () => {
		expect(() => getSegmentName('seg-$Number.m4s', 0, 1, 1000)).toThrow(/unbalanced/);
		expect(() => substituteTemplate('seg-$Number.m4s', { number: 1 })).toThrow(/unbalanced/);
	});
});
