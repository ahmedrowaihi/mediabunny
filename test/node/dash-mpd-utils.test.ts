...
describe('getCodecs / getBaseCodec / getSupplementalCodecs / getSupplementalProfiles', () => {
	...
	// Shaka asserts one media type here and would reject video+audio outright. We deliberately do
	// not — see `isMultiplexed`. Kept as its own case so a future shaka sync can still diff the port
	// against the original expectation. Verified playing in Shaka 5.2.9, dash.js 5.2.1, hls.js 1.7.2.
	test('getCodecs lists every codec of a multiplexed representation, video first', () => {
		expect(getCodecs({
	...
	test('getCodecs keeps the legacy WebM vp8 rewrite when multiplexed', () => {
		expect(getCodecs({
			containerType: 'webm',
			videoInfo: { codec: 'vp08.00.10.08' },
	...
	test('getCodecs still rejects text alongside either media type', () => {
		expect(() => getCodecs({
			videoInfo: { codec: 'avc1' },
			textInfo: { codec: 'wvtt' },
		})).toThrow();
	...
	test('getAdaptationSetKey keeps a multiplexed representation out of a video-only set', () => {
		// Sharing a set would let a bitrate switch drop the audio track mid-stream.
		const muxed = getAdaptationSetKey({
			containerType: 'mp4',
			videoInfo: { codec: 'avc1.640028' },
			audioInfo: { codec: 'mp4a.40.2' },
		}, false);
		const videoOnly = getAdaptationSetKey({
			containerType: 'mp4',
			videoInfo: { codec: 'avc1.640028' },
		}, false);

		expect(muxed).not.toBe(videoOnly);
	});
});
