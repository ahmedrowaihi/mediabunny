 ...
 * @public
 */
export const encryptHlsAes128 = (segment: Uint8Array, options: HlsAes128Options): Uint8Array => {
	const context = new Aes128CbcContext();
	context.init({ key: options.key, iv: options.iv });
 ...
 * @public
 */
export const buildHlsAes128KeyTag = (options: {
	/** `URI` the 16-byte key is fetched from. */
	uri: string;
	/** `IV` for the first segment. Without it, players derive one from the media sequence number. */
	iv?: Uint8Array;
	/** `KEYFORMAT`. Defaults to `identity`, which is what AES-128 normally uses. */
	keyFormat?: string;
}): string => {
	const attributes = ['METHOD=AES-128', `URI="${options.uri}"`];
	if (options.iv !== undefined) {
		attributes.push(`IV=0x${toHex(options.iv)}`);
	}
	if (options.keyFormat !== undefined) {
		attributes.push(`KEYFORMAT="${options.keyFormat}"`);
	}
