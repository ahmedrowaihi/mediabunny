...
export type TextInfo = {
	/**
	 * Whether the track is a forced narrative subtitle, which a player shows without the viewer asking.
	 * Beyond shaka's `MediaInfo.TextInfo`, which has no field for it.
	 */
	forced?: boolean;
...
/**
 * Whether one Representation carries both video and audio — a multiplexed Representation.
 *
 * DELIBERATE DEVIATION from shaka-packager, which always demuxes and so admits exactly one media
 * type per `MediaInfo`. We can emit a single file holding both, and DASH describes that by listing
 * every codec on one Representation. Text is addressed as its own AdaptationSet and never joins.
 *
 * Known ceiling: such an AdaptationSet still reports `contentType="video"`, so a player offering an
 * audio-track menu finds no audio AdaptationSet to list. `<ContentComponent>` is the spec's answer,
 * but DVB's DASH profile lets players ignore AdaptationSets carrying it, which trades a cosmetic
 * mislabel for a possible silent no-play. Emit one media type per file if that menu matters.
 *
 * @group DASH
 * @public
 */
export const isMultiplexed = (mediaInfo: MediaInfo): boolean =>
	mediaInfo.videoInfo !== undefined && mediaInfo.audioInfo !== undefined;
