...
export type EncryptWebmOptions = {
	/**
	 * The 16-byte AES-128 content encryption key. Unused when {@link EncryptWebmOptions.trackKeys} is
	 * given.
	 */
	key: Uint8Array;
	/**
	 * The key ID written to each encrypted track's `ContentEncKeyID`. Unused when
	 * {@link EncryptWebmOptions.trackKeys} is given.
	 */
	kid: Uint8Array;
	/** The initial 8-byte per-frame IV; each frame increments it by one (per track). */
	iv: Uint8Array;
	/**
	 * Per-track key material, keyed by `TrackNumber`, for encrypting each track under its own key
	 * instead of the whole file under `key`/`kid`. When given it must name every encryptable track of
	 * the file and no others: a track it omits is an error, not a fall back to `key`/`kid`. `iv`
	 * remains the IV of any entry that states none.
	 */
	trackKeys?: Map<number, WebmTrackKey>;
};

/**
 * The key material of one track, named by its `TrackNumber` in {@link EncryptWebmOptions.trackKeys}.
 *
 * @group Encryption
 * @public
 */
export type WebmTrackKey = {
	/** The 16-byte AES-128 content encryption key for this track. */
	key: Uint8Array;
	/** The key ID written to this track's `ContentEncKeyID`. */
	kid: Uint8Array;
	/**
	 * This track's initial 8-byte per-frame IV. Omit to use {@link EncryptWebmOptions.iv}; when
	 * stated it must be exactly 8 bytes, the fixed width of the IV field in a WebM encrypted frame
	 * header.
	 */
	iv?: Uint8Array;
};
 ...
 * @public
 */
export const encryptWebm = (bytes: Uint8Array, options: EncryptWebmOptions): Uint8Array => {
	const nodes = parseEbml(bytes, 0, bytes.length);
	const tracks = encryptableTracks(requireSegment(nodes));
	const material = keyMaterial(tracks, options);
	const encryptors = new Map<number, TrackEncryptor>();
	for (const { entry, meta } of tracks) {
		const { key, kid, iv } = material.get(meta.trackNumber)!;
		addContentEncryption(entry, kid);
		encryptors.set(meta.trackNumber, buildTrackEncryptor(meta, key, iv));
	}
 ...
 * @public
 */
export const encryptWebmInit = (init: Uint8Array, options: EncryptWebmOptions): Uint8Array => {
	const nodes = parseEbml(init, 0, init.length);
	const tracks = encryptableTracks(requireSegment(nodes));
	const material = keyMaterial(tracks, options);
	for (const { entry, meta } of tracks) {
		addContentEncryption(entry, material.get(meta.trackNumber)!.kid);
	}
	return serializeEbml(nodes);
};
 ...
 * init segment, which supplies the track/codec info.
 *
 * @group Encryption
 * @public
 */
export const encryptWebmSegment = (
	...
	options: EncryptWebmOptions & {
		 ...
		 * holds nothing but bytes, so it can be serialised and restored.
		 */
		ivState: Map<number, Uint8Array>;
	},
): Uint8Array => {
	if (options.ivState === undefined) {
		throw new Error('encryptWebmSegment needs an ivState to carry each track\'s IV across segments.');
	}

	const tracks = encryptableTracks(requireSegment(parseEbml(init, 0, init.length)));
	const material = keyMaterial(tracks, options);
	const encryptors = new Map<number, TrackEncryptor>();
	for (const { meta } of tracks) {
		const { key, iv } = material.get(meta.trackNumber)!;
		encryptors.set(meta.trackNumber, buildTrackEncryptor(meta, key, options.ivState.get(meta.trackNumber) ?? iv));
	}
	...
	for (const [trackNumber, encryptor] of encryptors) {
		options.ivState.set(trackNumber, encryptor.nextIv());
	}
	return serializeEbml(nodes);
};
...
type EncryptableTrack = { entry: EbmlNode; meta: TrackMeta };

const encryptableTracks = (segment: EbmlNode): EncryptableTrack[] => {
	const tracks: EncryptableTrack[] = [];
	forEachTrackEntry(segment, (entry) => {
		const meta = trackMeta(entry);
		if (meta !== null) {
			tracks.push({ entry, meta });
		}
	});
	return tracks;
};

type TrackKeyMaterial = { key: Uint8Array; kid: Uint8Array; iv: Uint8Array };

const keyMaterial = (tracks: EncryptableTrack[], options: EncryptWebmOptions): Map<number, TrackKeyMaterial> => {
	const material = new Map<number, TrackKeyMaterial>();
	const fileIv = options.iv.slice(0, WEBM_IV_SIZE);
	if (options.trackKeys === undefined) {
		for (const { meta } of tracks) {
			material.set(meta.trackNumber, { key: options.key, kid: options.kid, iv: fileIv });
		}
		return material;
	}

	const trackNumbers = tracks.map(track => track.meta.trackNumber);
	const unknown = [...options.trackKeys.keys()].filter(number => !trackNumbers.includes(number));
	if (unknown.length > 0) {
		throw new Error(
			`trackKeys names track ${unknown.join(', ')}, which this file has no encryptable track for;`
			+ ` its encryptable tracks are ${trackNumbers.join(', ')}.`,
		);
	}

	for (const { meta } of tracks) {
		const stated = options.trackKeys.get(meta.trackNumber);
		if (stated === undefined) {
			throw new Error(
				`trackKeys states no key for track ${meta.trackNumber}; give every encryptable track`
				+ ` (${trackNumbers.join(', ')}) an entry, or omit trackKeys to encrypt them all under key/kid.`,
			);
		}
		// Unlike CMAF, where one IV size is declared per file in `tenc`, a WebM encrypted frame header
		// carries a fixed-width IV, so a per-track IV is checked against the format, not against `iv`.
		if (stated.iv !== undefined && stated.iv.length !== WEBM_IV_SIZE) {
			throw new Error(
				`trackKeys states a ${stated.iv.length}-byte IV for track ${meta.trackNumber}; a WebM`
				+ ` encrypted frame header states its IV in exactly ${WEBM_IV_SIZE} bytes, so make it that long.`,
			);
		}
		material.set(meta.trackNumber, { key: stated.key, kid: stated.kid, iv: stated.iv ?? fileIv });
	}
	return material;
};

const buildTrackEncryptor = (meta: TrackMeta, key: Uint8Array, iv: Uint8Array): TrackEncryptor => {
	const videoCodec = meta.trackType === TRACK_TYPE_VIDEO ? codecOf(meta.codecId) : null;
	return videoCodec !== null
		? subsampleEncryptor(meta.trackNumber, videoCodec, key, iv)
		: wholeFrameEncryptor(meta.trackNumber, key, iv);
};

// VP9/AV1: reuse SampleEncryptor (cenc = AES-CTR + the ported subsample generators), then WebM-frame.
const subsampleEncryptor = (
	trackNumber: number, codec: EncryptionCodec, key: Uint8Array, iv: Uint8Array,
): TrackEncryptor => {
	...
	return {
		...
		nextIv: () => sampleEncryptor.nextIv(),
	};
};

// Audio / other tracks: whole-frame AES-CTR with an 8-byte IV incremented per frame.
const wholeFrameEncryptor = (trackNumber: number, key: Uint8Array, iv: Uint8Array): TrackEncryptor => {
	...
	return {
		...
		nextIv: () => new Uint8Array(ctr.getIv()),
	};
};
