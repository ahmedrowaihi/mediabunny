 ...
 */
/** @internal */
export const collectNamespaceFromName = (name: string, namespaces: Set<string>): void => {
...
export class XmlNode {
	...
	addElements(elements: Element[]): void {
		for (const element of elements) {
			...
			for (const [key, value] of element.attributes) {
				childNode.setStringAttribute(key, value);
			}
			childNode.setContent(element.content);
			childNode.addElements(element.subelements);
	...
	setIntegerAttribute(name: string, number: number): void {
