...
import { CBCS_HLS_METHOD, dashContentProtectionSpecs, hlsKeySpecs } from './manifest-protection';
import type { DrmSystem } from './manifest-protection';
...
const dashRendition = (rep: MpdRepresentation, set: MpdAdaptationSet): Rendition => ({
	...
	width: rep.width ?? set.width,
	height: rep.height ?? set.height,
	...
	sampleRate: rep.audioSamplingRate ?? set.audioSamplingRate,
	language: set.lang,
...
// A bare fourcc matches the entry's leading fourcc; anything carrying a dot must equal the whole entry,
// which separates AAC-LC (`mp4a.40.2`) from both HE-AAC profiles (`mp4a.40.5`, `mp4a.40.29`) — all `mp4a`.
const codecsInclude = (codecs: string | null, wanted: Set<string>): boolean => {
	if (codecs === null) {
		return false;
	}

	return codecs.split(',').some((raw) => {
		const entry = raw.trim().toLowerCase();
		if (wanted.has(entry.split('.')[0]!)) {
			return true;
		}
		return wanted.has(entry);
	});
};

/**
 * Drop every rendition whose codec matches one of `fourccs` (e.g. `['hev1', 'hvc1']` to keep an
 * AVC-only ladder). A bare fourcc matches any profile of it; a value carrying a dot (`'mp4a.40.5'`)
 * matches that exact codec string and nothing else. Matching is per-entry, so a muxed HLS variant `CODECS="avc1,mp4a"`
 * is dropped if *either* entry matches.
 *
 * @param kind - Only weigh renditions of this kind; every other kind passes through untouched. Without
 * it a muxed variant can be dropped by an entry you meant for one track type only.
 * @group Manifest
 * @public
 */
export const dropCodecs = (fourccs: string[], kind?: Rendition['kind']): ManifestTransform => {
	const wanted = new Set(fourccs.map(c => c.toLowerCase()));
	return filterRenditions(rendition =>
		outOfScope(rendition, kind) || !codecsInclude(rendition.codecs, wanted),
	);
};

/**
 * Keep only renditions whose codec matches one of `fourccs` (e.g. `['avc1', 'mp4a']` for an
 * H.264/AAC-only ladder). A bare fourcc matches any profile of it; a value carrying a dot
 * (`'mp4a.40.2'`) matches that exact codec string, so asking for AAC-LC does not also keep
 * `'mp4a.40.29'` (HE-AAC v2).
 * Renditions with no codec info (subtitles) are kept.
 *
 * @param kind - Only weigh renditions of this kind; every other kind passes through untouched. State
 * it whenever the list names one track type: `keepCodecs(['mp4a.40.2'])` drops every video rendition,
 * since no video codec appears in an audio list, while `keepCodecs(['mp4a.40.2'], 'audio')` does not.
 * @group Manifest
 * @public
 */
export const keepCodecs = (fourccs: string[], kind?: Rendition['kind']): ManifestTransform => {
	const wanted = new Set(fourccs.map(c => c.toLowerCase()));
	return filterRenditions(rendition =>
		outOfScope(rendition, kind) || rendition.codecs === null || codecsInclude(rendition.codecs, wanted),
	);
};
...
export type RangeFilter = {
	...
	max?: number;
	/**
	 * Only weigh renditions of this kind; every other kind passes through untouched. Without it the bound
	 * applies to every kind, so a video floor would also drop audio renditions whose bitrate sits below it.
	 */
	kind?: Rendition['kind'];
};

/** A resolution bound in pixels; omit a side to leave it open. @group Manifest @public */
export type ResolutionCap = {
	...
	maxHeight?: number;
	/** Minimum width in pixels, or unbounded when omitted. */
	minWidth?: number;
	/** Minimum height in pixels, or unbounded when omitted. */
	minHeight?: number;
};

// A rendition the bound does not speak about is kept, whether because its kind is out of scope or
// because it states no value to weigh.
const outOfScope = (rendition: Rendition, kind: Rendition['kind'] | undefined) =>
	kind !== undefined && rendition.kind !== kind;

/**
 * Keep only video renditions inside a resolution bound (audio/subtitle renditions are kept). Provide a
 * ceiling, a floor, or both — a rendition is kept when it has no height (non-video) or fits every stated
 * side, so `capResolution({ minHeight: 480, maxHeight: 1080 })` serves a middle ladder.
 *
 * @group Manifest
 * @public
 */
export const capResolution = (limits: ResolutionCap): ManifestTransform => {
	...
	const minWidth = limits.minWidth ?? 0;
	const minHeight = limits.minHeight ?? 0;
	return filterRenditions(r => r.height === null || (
		(r.width ?? 0) <= maxWidth && r.height <= maxHeight
		&& (r.width ?? Infinity) >= minWidth && r.height >= minHeight
	));
};
...
export const filterBitrate = (range: RangeFilter): ManifestTransform => {
	...
	return filterRenditions(r =>
		outOfScope(r, range.kind) || r.bandwidth === null || (r.bandwidth >= min && r.bandwidth <= max),
	);
};
...
export const filterFramerate = (range: RangeFilter): ManifestTransform => {
	...
	const max = range.max ?? Infinity;
	return filterRenditions(r =>
		outOfScope(r, range.kind) || r.frameRate === null || (r.frameRate >= min && r.frameRate <= max),
	);
};
...
export const filterChannels = (range: RangeFilter): ManifestTransform => {
	...
	const max = range.max ?? Infinity;
	return filterRenditions(r =>
		outOfScope(r, range.kind) || r.channels === null || (r.channels >= min && r.channels <= max),
	);
};

/**
 * Drop renditions whose video dynamic range is one of `ranges` — e.g. serve an SDR-only device an
 * HDR-free ladder with `dropByColorRange(['PQ', 'HLG'])`. Renditions with no signaled range (audio,
 ...
 * `#EXT-X-KEY` when it carries a `pssh`). A listed system that cannot be signalled on the manifest
 * being transformed makes {@link drm} throw rather than silently emit less — except FairPlay on
 * DASH, which has no DASH descriptor and is dropped there by design.
 *
 * @group Manifest
 * @public
 */
export type DrmOptions = {
	...
	systems: DrmSystem[];
	/** HLS FairPlay key URI (e.g. `skd://<kid>`); required whenever `systems` lists FairPlay. */
 ...
 *
 * @throws on an HLS media playlist, when a listed system cannot be signalled: FairPlay
 * without `fairplayKeyUri`, any other system without a `pssh`, or a PlayReady `pssh` that is not a
 * parseable `pssh` box.
 *
 * @group Manifest
 * @public
