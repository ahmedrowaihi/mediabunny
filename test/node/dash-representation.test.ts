...
describe('Representation — init() validation', () => {
	...
	// shaka: TEST_F(RepresentationTest, VideoAndAudioInfoSet) — shaka expects false here, because it
	// always demuxes. We deliberately accept it as a multiplexed Representation; see `isMultiplexed`.
	// This is the assertion that fails if a rebase reinstates shaka's one-media-type rule.
	test('init() accepts a multiplexed representation carrying both video and audio', () => {
		const mediaInfo: MediaInfo = {
			videoInfo: { codec: 'avc1', width: 640, height: 480, timeScale: 10, frameDuration: 10 },
	...
	test('init() still rejects text alongside video', () => {
		const mediaInfo: MediaInfo = {
			videoInfo: { codec: 'avc1', width: 640, height: 480, timeScale: 10, frameDuration: 10 },
			textInfo: { codec: 'wvtt' },
			containerType: 'mp4',
